...
@section('content')
    <div class="row">
        <div class="col">
            @if (!$posts->isEmpty())
                <div class="card">
                    <legend class="card-header text-center">All Articles</legend>
                    <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($posts as $post)
                                    <tr>
                                        ...
                                        <td>
                                            @if (($post->author === Auth::user()->student_id) || Auth::user()->hasPermissionTo('administer site content'))
                                                <a href="{{ route('posts.edit', $post->slug) }}" class="btn btn-raised btn-sm btn-info">Edit</a>
            ...
            @else
                <legend class="text-center mt-5">There are no posts available</legend>
                <p class="text-center">
                    <a href="{{ route('posts.create') }}" class="btn btn-raised btn-primary mt-5">
                        Add an article
                    </a>
                </p>
            @endif
